...
def create_standard_metadata_folder(base_path, objects, access_token, instance_url):
    ...
    for obj in objects:
        ...
        try:
            retrieve_id = retrieve_layout_metadata(access_token, instance_url, layout_name)
            for _ in range(10):
        ...
        merged_layout = merge_fields_into_layout(layout_xml, [f['apiName'] for f in fields])
        with open(os.path.join(layout_folder, f"{layout_name}.layout"), "w", encoding="utf-8") as f:
            f.write(merged_layout)

        object_xml = generate_object_file(object_name, fields)
        with open(os.path.join(object_folder, f"{object_name}.object"), "w", encoding="utf-8") as f:
            f.write(object_xml)

        profile_xml = generate_profile_xml(object_name, fields)
        with open(os.path.join(profile_folder, "Admin.profile-meta.xml"), "w", encoding="utf-8") as f:
            f.write(profile_xml)
